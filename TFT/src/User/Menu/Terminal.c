...
void terminalCache(const char * stream, uint16_t streamLen, SERIAL_PORT_INDEX portIndex, TERMINAL_SRC src)
{
  ...
  char * srcID[_TERMINAL_COUNT] = {"\5", "\6"};
...
{
  TERMINAL_SRC lastSrc = _TERMINAL_GCODE;
  char * endPtr = (ptr + 1);
...
void menuTerminalWindow(void)
{
  ...
  KEY_VALUES key_num = KEY_IDLE;
  CHAR_INFO info;
  TERMINAL_SRC pageHeadSrc = _TERMINAL_GCODE;
  ...
  terminalData->lastSrc = (_TERMINAL_ACK + 1);
...
{
  TERMINAL_DATA termPage = {{terminalBuf}, 0, 0, 0, 0, 0, 0, TERMINAL_MAX_CHAR, MAX_PAGE_COUNT, 0, (_TERMINAL_ACK + 1)};
