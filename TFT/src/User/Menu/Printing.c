...
void printingDrawPage(void)
{
  ...
  gn = strlen((char *)getCurGcodeName(infoFile.title)) - 6; // -6 means ".gcode"
  gnew = malloc(gn + 10);
  if(gnew)
  {
    strcpy(gnew, getCurFileSource());
    strncat(gnew, (char *)getCurGcodeName(infoFile.title), gn);
    if(bmpDecode(strcat(gnew, "_"STRINGIFY(ICON_WIDTH)".bmp"), ICON_ADDR(ICON_PREVIEW)))
    {
      lcd_frame_display(1*ICON_WIDTH+1*SPACE_X+START_X,0*ICON_HEIGHT+0*SPACE_Y+ICON_START_Y,ICON_WIDTH,ICON_HEIGHT,ICON_ADDR(ICON_PREVIEW));
    }
      free(gnew);
  }
}
