...
void updateFeatureSettings(uint8_t item_index)
{
  switch (item_index)
  {
    ...
    case SKEY_EMULATED_M109_M190:
      TOGGLE_BIT(infoSettings.general_settings, ((item_index - SKEY_EMULATED_M600) + _EMULATED_M600));
      break;
...
void loadFeatureSettings(LISTITEM * item, uint16_t item_index, uint8_t itemPos)
{
  if (item_index < SKEY_COUNT)
  {
    switch (item_index)
    {
      case SKEY_EMULATED_M600:
      case SKEY_EMULATED_M109_M190:
        item->icon = iconToggle[GET_BIT(infoSettings.general_settings, ((item_index - SKEY_EMULATED_M600) + _EMULATED_M600))];
        break;
