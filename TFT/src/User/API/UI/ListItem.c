...
static const char * dynamic_label[LISTITEM_PER_PAGE];
static char dynamic_text_value[LISTITEM_PER_PAGE][MAX_TEXT_LEN];

void setDynamicLabel(uint8_t i, const char * label)
{
  dynamic_label[i] = label;
}

// get dynamic text label ( i : index of the label position)
const char * getDynamicLabel(uint8_t i)
{
...
void setDynamicTextValue(uint8_t i, const char * txt)
{
  //dynamic_text_value[i] = txt;
  strncpy_no_pad(dynamic_text_value[i], txt, (MAX_TEXT_LEN - 2));
}

// get dynamic text value ( i : index of the text value position)
const char * getDynamicTextValue(uint8_t i)
{
...
{
  snprintf(dynamic_text_value[i], (MAX_TEXT_LEN - 2), "%.5f", value);
}
...
static void draw_itemtitle(GUI_POINT pos, LABEL label, uint8_t position, int textarea_width)
{
  if (label.index != LABEL_NULL)
  {
    ...
    if (label.index == LABEL_DYNAMIC)
      ...
      GUI_DispLenString(pos.x, pos.y, (uint8_t *) labelGetAddress(&label), textarea_width, true);
  }
}
...
static inline void ListItem_DisplayToggle(uint16_t sx, uint16_t sy, uint8_t iconchar_state)
{
  ...
  GUI_SetColor(infoSettings.list_border_color);
  _GUI_DispString(sx, sy, IconCharSelect(CHARICON_TOGGLE_BODY));
