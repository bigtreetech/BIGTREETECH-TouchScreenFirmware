...
//  GUI_DrawPixel(x, y, color);
  GUI_FillRectColor(infoSettings.marlin_mode_fullscreen ? SIMULATOR_XSTART_FULLSCREEN + PIXEL_XSIZE_FULLSCREEN*x :
                    SIMULATOR_XSTART + PIXEL_XSIZE*x,
                    infoSettings.marlin_mode_fullscreen ? SIMULATOR_YSTART_FULLSCREEN + PIXEL_YSIZE_FULLSCREEN*y : 
                    SIMULATOR_YSTART + PIXEL_YSIZE*y ,
                    infoSettings.marlin_mode_fullscreen ? SIMULATOR_XSTART_FULLSCREEN + PIXEL_XSIZE_FULLSCREEN*(x+1) :
                    SIMULATOR_XSTART + PIXEL_XSIZE*(x+1) ,
                    infoSettings.marlin_mode_fullscreen ? SIMULATOR_YSTART_FULLSCREEN + PIXEL_YSIZE_FULLSCREEN*(y+1) :
                    SIMULATOR_YSTART + PIXEL_YSIZE*(y+1), 
                    color);
}
