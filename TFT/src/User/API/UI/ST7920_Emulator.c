...
static const ST7920_POSITION st7920_init_position = {
...
static const ST7920_REG st7920_inti_reg = {
...
static const float st7920_gx_start_full = ST7920_GXSTART_FULLSCREEN;
static const float st7920_gy_start_full = ST7920_GYSTART_FULLSCREEN;
static const float st7920_gx_dot_full   = ST7920_GXDOT_FULLSCREEN;
static const float st7920_gy_dot_full   = ST7920_GYDOT_FULLSCREEN;
static const uint16_t st7920_gx_start   = ST7920_GXSTART;
static const uint16_t st7920_gy_start   = ST7920_GYSTART;
static const uint16_t st7920_gx_dot     = ST7920_GXDOT;
static const uint16_t st7920_gy_dot     = ST7920_GYDOT;

static ST7920 * pSt7920;

static inline void ST7920_DrawPixel(int16_t x, int16_t y, bool isForeGround)
{
...
static void ST7920_ClearRAM(void)
{
  // Clear CGRAM buffer
...
static void ST7920_CursorIncrease(void)
{
  ...
  if (pSt7920->position.xByte >= 16)
  {
    ...
    if (pSt7920->position.yPixel == 64)
      pSt7920->position.yPixel = 16;
    else if (pSt7920->position.yPixel == 80)
      pSt7920->position.yPixel = 0;
  }
}

// Draw the byte at the specified position.
// The byte comes from the XOR of DDRAM(CGRAM or HCGROM) and GDRAM
static void ST7920_DrawByte(uint8_t xByte, uint8_t yPixel)
{
...
// Display graphic
static inline void ST7920_DrawGDRAM(uint8_t data)
{
  if (pSt7920->reg.fs.g != 1)  // Graphic display off
    return;
...
static inline void ST7920_DrawDDRAM(uint8_t data)
{
...
// cmd : 1 << 5
static void ST7920_CI15_FunctionSet(uint8_t cmd)
{
...
// cmd : 1 << 0
static inline void ST7920_BI10_DisplayClear(uint8_t cmd)
{
...
// cmd : 1 << 1
static inline void ST7920_BI11_ReturnHome(uint8_t cmd)
{
...
// cmd : 1 << 2
static inline void ST7920_BI12_EntryModeSet(uint8_t cmd)
{
  pSt7920->reg.bi.ems.reg = cmd;
}

// cmd : 1 << 3
static inline void ST7920_BI13_DisplayControl(uint8_t cmd)
{
  pSt7920->reg.bi.dc.reg = cmd;
}

// cmd : 1 << 4
static inline void ST7920_BI14_CursorDisplayControl(uint8_t cmd)
{
  pSt7920->reg.bi.cdsc.reg = cmd;
}

// cmd : 1 << 6
static inline void ST7920_BI16_SetCGRAMAddress(uint8_t cmd)
{
...
static inline void ST7920_BI_SetCGRAMData(uint8_t data)
{
...
// cmd : 1 << 7
static inline void ST7920_BI17_SetDDRAMAddress(uint8_t cmd)
{
...
// cmd : 1 << 0
static inline void ST7920_EI10_StandBy(uint8_t cmd)
{
  pSt7920->reg.ei.sb.reg = cmd;
}

// cmd : 1 << 1
static inline void ST7920_EI11_ScrollOrRAMAddress(uint8_t cmd)
{
  pSt7920->reg.ei.vsra.reg = cmd;
}

// cmd : 1 << 2
static inline void ST7920_EI12_Reverse(uint8_t cmd)
{
...
// cmd : 1 << 6
static inline void ST7920_EI16_SetScrollAddress(uint8_t cmd)
{
  pSt7920->reg.ei.sa.reg = cmd;
}

// cmd : 1 << 7
static inline void ST7920_EI17_SetGDRAMAddress(uint8_t cmd)
{
...
static inline void ST7920_ST7920_ParseWCmd(uint8_t cmd)
{
...
static inline void ST7920_ST7920_ParseWData(uint8_t data)
{
...
static inline uint8_t ST7920_IsCtrlByte(uint8_t data)
{
