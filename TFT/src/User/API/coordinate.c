...
void coordinateSetExtruderActual(float position)
{
  curPosition.axis[E_AXIS] = extruderPostion = position;
//  curPosition.axis[E_AXIS] = extruderPostion = position / getParameter(P_STEPS_PER_MM, E_AXIS);
}
...
void coordinateQuery(void)
{
  ...
  {
    coordinateQueryWait = storeCmd("M114 R\n");
  }
}
