...
bool sendCmd(bool purge, bool avoidTerminal)
{
  ...
  if (GET_BIT(infoSettings.general_settings, _LISTENING_MODE) == 1 &&  // if TFT is in listening mode and FW type was already detected,
  ...
  {
    if (purge)
      terminalCache(purgeStr, strlen(purgeStr), cmd_port_index, _TERMINAL_GCODE);
    terminalCache(cmd_ptr, cmd_len, cmd_port_index, _TERMINAL_GCODE);
  }
    ...
    case 'M':
      ...
      switch (cmd)
      {
        ...
        case 109: // M109
          if (fromTFT)
          {
            if (GET_BIT(infoSettings.general_settings, _EMULATED_M109_M190) == 0)  // if emulated M109 / M190 is disabled
              break;
        ...
        case 600:  // M600 filament change
          if (isPrinting())
          {
            ...
            if (GET_BIT(infoSettings.general_settings, _EMULATED_M600) == 1)
            {
              sendCmd(true, avoid_terminal);
              printPause(true, PAUSE_NORMAL);
              return;
