 ...
 * - Time mode is very close to the real amount of work done, but it is still not perfect, it relies
 *   on the estimate the slicer has done. It needs info from the slicer such as the elapsed time or
 *   the remaining time. If that info is missing, the progress source defaults to file mode.
 *   If no "M73 R" is present in the G-code file then it requires "file_comment_parsing" to be enabled.
 *
 * NOTE: If "M73 P" is present in the G-code file then file or time based progress modes will be
 *       overriden by that.
 *
 ...
 *
 * NOTE: Some slicers include time related info in the G-code files as comments. If the time info is
 *       not added by the slicer, then a post processing plugin is needed that adds that info in the
 *       G-code file (comment or M73 Rxx format).
