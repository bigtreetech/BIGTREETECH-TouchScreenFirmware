...
#define BUZZER_CACHE_SIZE 8  // queue to store 8 sounds (1 sound takes 4 bytes)
...
void TIM3_IRQHandler(void) // ====== GD32F2XX TIMER ISR ======
{
...
{
  if (duration == 0) return; // nothing to do, frequency == 0 indicates silence

  if (((buzzer.wIndex + 1) % BUZZER_CACHE_SIZE) == buzzer.rIndex)
    return; // queue is full, drop sound

// OLD CODE:
//   while (buzzer.count == BUZZER_CACHE_SIZE)
//   {
//     loopBuzzer(); // GREAT IDEA, PAUSE THE PRINTER TO LISTEN TO THE SOUNDS
//   }
...
void loopBuzzer(void)
{
  ...
  if (buzzer.rIndex != buzzer.wIndex) // play queued sound
  {
    ...
    if ((freq == SOUND_SILENCE) || (freq > SOUND_KEYPRESS))
    {
      tone(freq, duration);
    }
    else
